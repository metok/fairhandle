...
describe('E2E deadlock (best_effort)', () => {
  it('transitions both peers to closing after 3 consecutive disputes', async () => {
    ...
    const llmA = new ScriptedLLMAdapter({ consolidatorOutputs: [divA(1), divA(2), divA(3)], verifierAlways: { equivalent: true }, artifactEquivalence: dispute })
    const llmB = new ScriptedLLMAdapter({ consolidatorOutputs: [divB(1), divB(2), divB(3)], verifierAlways: { equivalent: true }, artifactEquivalence: dispute })
